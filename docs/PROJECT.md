...
- **D16: Manifest name/short_name/description are localized via Chrome's native `_locales` + `__MSG__`** — The popup/overlay runtime UI keeps its own dictionary (`extension/src/i18n.ts`) because `chrome.i18n` is locked to the browser UI language and we want a user-selectable override. But the *manifest* fields shown in chrome://extensions and the Web Store can't use that runtime system, so they use the standard mechanism: `default_locale:"en"` + `_locales/{en,ja,zh}/messages.json` (keys `appName`/`appShortName`/`appDescription`) + `__MSG_*__` placeholders. `build.mjs` copies `_locales` into `dist`; the store/release zips pick it up automatically. Descriptions kept ≤132 chars and lead with the two differentiators (works where official browser tools refuse; token-efficient + fast). (2026-06-18)
...
- **D27: Connected profile is identified by its signed-in account email, pin overrides focus-follow, popup is i18n'd (en/ja/zh)** — Building on D12 (focus-follow + evicted/suppress), three additions for the multi-Chrome-profile UX. (1) **Identity**: the extension sends its profile's signed-in account email (`chrome.identity.getProfileUserInfo`, `identity`/`identity.email` perms) in the `hello` frame; the server tracks it and `index.ts` appends a `[profile] <email>` sidecar to every tool result so the AI can tell the user which profile it's driving. Chrome exposes no API for the human profile name ("Work"/"Personal"), so email is the identifier. (2) **Pin**: a 📌 toggle in the popup's status row pins the connection to one browser. Arbitration moved server-side — the bridge decides ownership at `hello` time (not eagerly on socket open): a pinned holder refuses non-pin takeovers (`evicted` with `reason:"pinned"` → loser stays dormant, no focus-reclaim), while a pin claim overrides anything. New `pin` wire message toggles state without reconnecting. (3) **i18n**: `extension/src/i18n.ts` is a runtime dictionary (en/ja/zh) shared by popup + overlay; auto-detects from `chrome.i18n.getUILanguage()`, overridable via a popup language selector persisted under `lang`. Chrome's native i18n can't be switched at runtime, hence the custom dict. (2026-06-13)
