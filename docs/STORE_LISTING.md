...
server to Claude and sets up routing so Claude always uses Yolo for Chrome):
    ...
    claude mcp add --scope user yolo-chrome -- npx -y yolo-chrome-mcp@latest && npx -y yolo-chrome-mcp@latest install --routing-only
...
Privacy policy: {{PRIVACY_POLICY_URL — set up separately}}
```
...
```
{{PRIVACY_POLICY_URL — set up separately}}
```
